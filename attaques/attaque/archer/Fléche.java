...
import fr.pierrehb.entities.particles.Arrow;
...
public class Fléche extends Attack implements Color{
	float x1,x2,x3,x4,y1,y2,y3,y4;
	private static Sound_lector song;


	public Fléche(Human Joueur, int level) {
		super(Joueur, level);
		xo = 0.0f;
		yo = 2.0f;
		timeToDone = 25;
	...
	public void startAttack() {
		...
		Game.level.addParticle(new Arrow((int)joueur.x+(Human.taille[0]/2)-4, (int)joueur.y+(Human.taille[1]/2)-4, joueur.getAngle(), 2.0f, joueur, damageHeal+joueur.getDamage()));
		song = new Sound_lector("res/sound/sou/attaques/rafter/atk1.wav", Main.save.soundVolume);
		song.start();
	}
