...
public class Fire_Ball_Particle extends Mortal_Particle{
	private Pixel[] pixels = new Pixel[44];
	int damage = 3;


	
	public Fire_Ball_Particle(int X, int Y, float[] Speed, Entity joueur) {
		super(X, Y, 60, joueur, 6);
		this.speed[0] = Speed[0];
		this.speed[1] = Speed[1];
		ini();
	}
	public Fire_Ball_Particle(int X, int Y, float[] Speed, Entity joueur, int Damage) {
		super(X, Y, 60, joueur, 6);
		this.speed[0] = Speed[0];
		this.speed[1] = Speed[1];
		this.damage = Damage;
